	...
	if((player.pregnancyIncubation <= 0 && player.buttPregnancyIncubation <= 0) ||
		...
		return false;
			...
			if(player.pregnancyIncubation > 1) {
				...
				if(player.pregnancyType == PregnancyTypeEnum.SPIDER || player.pregnancyType == PregnancyTypeEnum.DRIDER) {	
					...
					if(player.pregnancyIncubation == 180) {
						...
						if(player.pregnancyType == PregnancyTypeEnum.SPIDER) outputText("spider-morphs ", false);
				...
				//Frog Eggs
				else if(player.pregnancyType == PregnancyTypeEnum.FROG_BUTT_EGG) {
			...
			if(player.buttPregnancyIncubation > 1) {
				if(player.buttPregnancyType == PregnancyTypeEnum.FROG_BUTT_EGG.val) {
				...
				//Pregnancy 4 Satyrs
				if(player.buttPregnancyType == PregnancyTypeEnum.SATYR.val) {
					//Stage 1: 
			...
			if(player.buttPregnancyIncubation == 1 && player.buttPregnancyType == PregnancyTypeEnum.FROG_BUTT_EGG.val) {
			...
			if(player.pregnancyType == PregnancyTypeEnum.PROPER_BASILISK && player.pregnancyIncubation <= 2) {
			...
			if(player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyTypeEnum.FROG_BUTT_EGG) {
			...
			{
				if(flags[kFLAGS.AMILY_FOLLOWER] == 2 || flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00170] > 0) player.pregnancyType = PregnancyTypeEnum.MOUSE;
			}
			//Amily failsafe - converts PC with pure babies to mouse babies if Amily is with Urta
			if(player.pregnancyIncubation == 1 && player.pregnancyType == PregnancyTypeEnum.AMILY) 
			{
				if(flags[kFLAGS.AMILY_VISITING_URTA] == 1 || flags[kFLAGS.AMILY_VISITING_URTA] == 2) player.pregnancyType = PregnancyTypeEnum.MOUSE;
			}
