	...
	//Repeat meetings
		...
		if(flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] < 20) {
			...
			if(!player.minotaurAddicted()) {
				...
				if(player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216 && player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("Pregnant with another brother for us, and already you come back for more?", false);
			...
			//[Addicted Repeat] 
				...
				if(player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216 && player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("Pregnant with another brother for us, and you're already back for another fix?  Why even leave in the first place?", false);
		...
		//Tribe sized
			...
			if(!player.minotaurAddicted()) {
				...
				else if(player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("Come back to show off the lil' bro yer growin'?", false);
			...
			//[Repeat Tribe-Size Addict] 
				...
				else if(player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("Come back for some more spunk? I bet your next son comes out as a spooge-slurping minitaur.  We could use another cock-sleeve around here.", false);
...
private function nonAddictMinotaurGangBang():void {
	...
	else if(player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("I hope you don't mind me saying hi to my little brother.", false);
	...
	//Preggers chance!
	player.knockUp(PregnancyTypeEnum.MINOTAUR,432,75);
...
private function loseToMinoMobVeryPregnant():void {
	...
	if(player.pregnancyType != PregnancyTypeEnum.MINOTAUR) outputText("She doesn't know how to keep her hungry twat closed.  You can see the vermin squirming in her belly.  Disgusting.", false);
	...
	if(player.pregnancyType != PregnancyTypeEnum.MINOTAUR) outputText("isn't", false);
	...
	if(player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("That baby is probably mine too.  You let your son father his brother.", false);
	...
	//(Long) 
		...
		if(player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("we wouldn't want the next member of our tribe injured.", false);
...
private function analSpearSemiPregMinotaurGangbang():void {
	...
	if(player.pregnancyType == PregnancyTypeEnum.MINOTAUR) outputText("our little brother", false);
