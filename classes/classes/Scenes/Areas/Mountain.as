...
package classes.Scenes.Areas
{
	...
	public class Mountain extends BaseContent
	{
		...
		public function exploreMountain():void
		{
			...
			if (rand(100) <= 77) {
				if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00141] < 3) {
					...
					if (player.faceType == FACE_DOG && (player.dogCocks() >= 2 || (player.hasVagina() && player.pregnancyType == PregnancyType.HELLHOUND && player.pregnancyIncubation > 0)) && player.cor >= 60 && player.tailType == TAIL_TYPE_DOG && (player.lowerBody == LOWER_BODY_TYPE_DOG || player.hairColor == "midnight black")) {
		...
		private function joinBeingAMinoCumSlut():void
		{
			...
			player.knockUp(PregnancyType.MINOTAUR, 432);
