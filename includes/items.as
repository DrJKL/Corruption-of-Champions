...
public function ovipositionElixer():void {
	...
	if(player.vaginas.length > 0) { 
		...
		if(changes == 0 && player.pregnancyIncubation > 20 && player.pregnancyType != PregnancyType.BUNNY) {
...
public function neonPinkEgg(pregnantChange:Boolean = false):void {
	...
	if(pregnantChange) {
		...
		if(player.buttPregnancyType == 9 && player.pregnancyType == PregnancyType.BUNNY) outputText("rumble", false);
	...
	if(player.statusAffectv2("heat") < 30 && rand(2) == 0 && changes < changeLimit && player.pregnancyIncubation == 0 && player.hasVagina()) {
		...
		if(player.hasStatusAffect("heat") >= 0) {
			...
			//(mino cum in inventory and non-horse, 100 lust)
				...
				player.knockUp(PregnancyType.MINOTAUR,432,175);
